...
## 3. Packet D: "The Delivery" (128 Bytes CCSDS / 136 Bytes SNLP)

_Generated by Sat B. Optimized for radio FIFO (Power of 2)._

> **Magic Byte (VOID-006, F-03, 2026-04-15):** Body offset 0 carries the
> fixed discriminant `MAGIC = 0xD0`. This closes the `dispatch_122`
> single-bit-flip collision with Packet ACK (`MAGIC = 0xAC`) — Hamming
> distance 4 guarantees no single RF bit-flip can transmute one type
> into the other. The byte is absorbed from the legacy 2-byte
> `_pad_head`; every downstream field offset and the total frame size
> are unchanged. Kaitai `dispatch_122` routes on this byte; the gateway
> performs an application-layer `raw[headerLen] == 0xD0` check before
> trusting the parser. Reject with HTTP 400 on any mismatch.

### 3.1 CCSDS Tier (128 Bytes)

| Offset      | Field        | Type     | Size | Endian     | Description                                  |
| ----------- | ------------ | -------- | ---- | ---------- | -------------------------------------------- |
| **00-05**   | `ccsds_pri`  | `u8[6]`  | 6B   | **Big**    | Header (APID = Sat B)                        |
| **06**      | `magic`      | `u8`     | 1B   | -          | **F-03 discriminant: `0xD0` (body offset 0)** |
| **07**      | `_pad_head`  | `u8`     | 1B   | -          | **Cycle Alignment Pad**                      |
...
### 3.2 SNLP Tier (136 Bytes)

Identical body layout to the CCSDS variant; the 14-byte SNLP header
simply shifts every body offset forward by 8 bytes. `magic` still sits
at body offset 0 (frame offset 14).

| Offset      | Field        | Type     | Size | Endian     | Description                                  |
| ----------- | ------------ | -------- | ---- | ---------- | -------------------------------------------- |
| **00-13**   | `snlp_hdr`   | `u8[14]` | 14B  | **Big**    | Sync + CCSDS + align_pad                     |
| **14**      | `magic`      | `u8`     | 1B   | -          | **F-03 discriminant: `0xD0` (body offset 0)** |
| **15**      | `_pad_head`  | `u8`     | 1B   | -          | **Cycle Alignment Pad**                      |
| **16-23**   | `ccsds_sec`  | `u64`    | 8B   | **Little** | Timestamp: Downlink Time                     |
| **24-27**   | `sat_b_id`   | `u32`    | 4B   | **Little** | Carrier ID (Sat B)                           |
| **28-125**  | `payload`    | `u8[98]` | 98B  | -          | **STRIPPED PACKET C** (Header removed)       |
| **126-129** | `global_crc` | `u32`    | 4B   | **Little** | Outer Integrity Check                        |
| **130-135** | `_tail`      | `u8[6]`  | 6B   | -          | **Final alignment to 136B**                  |
