...
| 6.6 | [VOID-142](https://www.notion.so/3dad64b77e4081f8952fd0697b1aef2b) | Ground Station UI live-data wiring (gateway + chain + contract, then serial) | In progress | P0 | **Inserted 2026-09-13 (v18) — user direction.** Wires the VOID-141 inert console to the flat-sat services per [PORT_SPEC](VOID-141_IMGUI_PORT_SPEC.md) §8. **Stage 1 (branch `void_142_part1`):** gateway `GET /api/v1/status` + counters, UI process manager (anvil/gateway spawn, START/STOP, LOAD NEXT CONTRACT via `forge create` per [README_GATEWAY.md](../gateway/README_GATEWAY.md)), live Panels 2/3 + `receipts.json` drawer tailer. **Stage 2 (after Stage-1 confirmation):** bouncer child + serial wiring — setvbuf fix, piped stdin/stdout, SEND ACK, leg FSM, `heartbeats.json` HB leg. Deferred: CONTRACT VALUE firmware plumbing. Parallel-ok with #7, #17 — desktop tooling, zero firmware/wire-format impact. |
