...
| 15 | [VOID-135](https://www.notion.so/344d64b77e40817f883ce2f70ed7fe76) + [VOID-138](https://www.notion.so/VOID-138-Bouncer-C-Egress-Poll-Client-347d64b77e40813386e0e5dcfc2bacc5) | Gateway Packet C Receipt + `receipts.json` + bouncer egress poll | 🟡 #15a + #15b-gw landed, #15c open (VOID-138) | P0 | Split across three PRs per Change Log v7 + v8. **#15a (merged):** in-process event watcher + PacketC builder + JSONL store. **#15b-gateway (merging):** HTTP egress endpoints (GET /pending, POST /ack), PENDING/DISPATCHED state machine, crash-safety across restart proven by 27 Go tests. **#15c = VOID-138 (open):** C++ bouncer poll client that consumes the gateway's egress contract, LoRa-TXes each PacketC, then ACKs. Row stays 🟡 until VOID-138 ships. **Still produces the TRL 4 artefact (`receipts.json`).** |
