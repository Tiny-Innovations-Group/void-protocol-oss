...
| 15 | [VOID-135](https://www.notion.so/344d64b77e40817f883ce2f70ed7fe76) | Gateway Packet C Receipt + `receipts.json` Persistence | 🟡 #15a landed, #15b open | P0 | **NEW (v4).** Split per Change Log v7. **#15a (merged):** event watcher + PacketC builder (byte-exact vs golden) + signed Ed25519 + JSONL persistence + reload-dedup on restart. All in-process; covered by `receipt` + `chain.Watcher` tests inc. live `simulated.Backend`. **#15b (open):** LoRa egress to bouncer + end-to-end crash-safety E2E test. Row stays open until #15b ships. **Still produces the TRL 4 artefact (`receipts.json`).** |
