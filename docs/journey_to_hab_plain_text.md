...
| 9 | [VOID-116](https://www.notion.so/342d64b77e4081f1840af2b05c401807) | Align RelayOps u32 Fields in PacketAck | Not started | P1 | **KSY hygiene — on path because ACK now flies.** Must land before #13 (ACK emission) so ACK struct is correctness-clean cross-endian. Regenerate golden vectors after. |
| 10 | [VOID-119](https://www.notion.so/342d64b77e4081beb7adc427af016de9) | Align PacketD global_crc | Not started | P1 | **KSY hygiene — on path because D now flies.** Must land before #16 (D emission). Regenerate golden vectors after. |
| 11 | [VOID-122](https://www.notion.so/342d64b77e408150bc66c313cdbcc0a5) | CRC Pre-Validation Before D/ACK Dispatch | Not started | P0 | **No longer hygiene — now a functional bug risk.** With both D and ACK on the wire, the type-bit dispatch disambiguation MUST run after a correct CRC or we silently route the wrong handler. Must land before #13 and #16. |
| 12 | [VOID-129](https://www.notion.so/344d64b77e40814ba8a9eaf9f58daf4f) | Wire Ground Bouncer → Gateway Signature Verify | Not started | P0 | Requires #5. **Full Ed25519 verify over VOID-111 scope — not stubbed, not bypassed.** Failed verify = dropped + logged. Feeds #13's ACK emit decision. |
| 13 | [VOID-134](https://www.notion.so/344d64b77e4081c399b2ecc9c1c2df62) | Ground Bouncer → Buyer ACK Emission (firmware) | Not started | P0 | **NEW (v4).** Requires #9, #11, #12. On successful PacketB verify, bouncer emits PacketAck back to the buyer within 500 ms. Unit test + golden vector `test/vectors/snlp/packet_ack.bin`. |
...
| 16 | [VOID-136](https://www.notion.so/344d64b77e40818ba490c2d7cbd637a4) | Seller Packet D Delivery Emission (firmware) | Not started | P0 | **NEW (v4).** Requires #10, #11, #15. On verified PacketC RX, seller emits PacketD to buyer confirming delivery. Unit test + golden vector `test/vectors/snlp/packet_d.bin`. |
