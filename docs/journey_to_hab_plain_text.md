...
| 6.5 | VOID-141 | ImGui Interactive Ground Station UI (4-window demo console) | In progress | P0 | **Inserted 2026-09-11 (v16) at top of the open list — user direction.** Host-side Dear ImGui operator console, 4 windows: live Pipeline (A→B→ACK→Settle→C→D + Heartbeat), Go Server (gateway status), L2 Blockchain (Anvil + `receipts.json` tail), Controls (Load Next Contract / Start / Stop / Shut Off). Needed as the observation surface for the flat-sat demo **screen recording**. Parallel-ok with #7 — desktop tooling only, zero firmware/wire-format impact. Full spec: [VOID-141_ImGui_Ground_Station_UI.md](VOID-141_ImGui_Ground_Station_UI.md). **Stage-1 inert port landed 2026-09-13 (Change Log v17):** single 1760×826 window, 4 fixed panels, per frozen [PORT_SPEC](VOID-141_IMGUI_PORT_SPEC.md); live-data wiring deferred to future tickets (spec §8). |
