...
| **C-02** | ~~WARNING~~ **RESOLVED** | CCSDS Compliance | `version` field parsed but never validated. Non-zero version = unknown protocol revision. | `valid: { eq: 0 }` is **not supported on KSY instance fields** (compiler error). Enforced application-side: gateway ingest handler checks `version == 0` immediately after parse and rejects with HTTP 400. Documented in `ccsds_primary_header.ksy` instance doc. |
