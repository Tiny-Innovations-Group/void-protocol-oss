...
void runSellerLoop() {
    ...
    // ---------------------------------------------------------
    // 1c. VOID-022: 30 s heartbeat telemetry (droppable, CAD-gated).
    //     Only when no RX is pending — TX shares the SX126x FIFO base
    //     with RX, so transmitting before draining a received frame
    //     would clobber it.
    // ---------------------------------------------------------
    if (!rx_flag) {
